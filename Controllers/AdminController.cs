    ...
    public class AdminController : Controller
    {
        ...
        public async Task<IActionResult> IndexAsync(string site, string filter, string direction, int? showModalId = null, int? showDownModalId = null, int? ShowMaterialModalId = null)
        {
            try {
                ...
                {
                    var job = jobs.Find(x => x.ID == showModalId);
                    ViewBag.Materials = await _orderService.GetProductionItemMaterials((int)showModalId, (int)ShowMaterialModalId, site, job.ProductionLines[0].ProductionScheduleID, job.ItemID, job.DueDate.ToString("MM/dd/yyyy"));
